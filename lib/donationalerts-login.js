...
async function storeUserDAConnection(userId, username) {
  try {
    await db.updateUserDAConnection(userId, username);
...
async function getUserDAUsername(userId) {
  try {
    return await db.getUserDAConnection(userId);
  } catch (error) {
