...
if (!redis || !subscriber) {
  console.warn('[DA] Redis is not available. Username cache will work locally only.');
}
...
async function initializeUsernameCache() {
  try {
    ...
    // Prime Redis asynchronously (only if Redis is available)
    if (redis) {
      (async () => {
