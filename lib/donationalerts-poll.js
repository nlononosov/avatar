...
async function refreshIfNeeded(creds) {
  ...
  if (creds.da_expires_at && now >= creds.da_expires_at - 60) {
    try {
      ...
      await upsertStreamerDA(newCreds);
...
async function processDonation(streamerId, donation) {
  try {
    ...
    if (await isDonationProcessed(streamerId, String(donation.id))) {
      console.log(`[DA Poll] Donation ${donation.id} already processed for streamer ${streamerId}`);
      return;
    }

    // Отмечаем как обработанный
    await markDonationProcessed(streamerId, String(donation.id));
    ...
    if (donation.user_id) {
      user = await findUserByDAUserId(String(donation.user_id));
      if (user) {
    ...
    if (!user && donation.username) {
      try {
        user = await findUserByNormalizedLogin(donation.username);
        if (user) {
    ...
    const avatar = await getAvatarByTwitchId(user.twitch_user_id);
    if (!avatar) {
    ...
    try {
      const success = await addUserToStreamer(user.twitch_user_id, streamerId);
...
async function pollStreamer(streamerId) {
  try {
    const creds = await getStreamerDA(streamerId);
...
async function refreshStreamersCache(force = false) {
  const now = Date.now();
  if (!force && now - lastRefreshAt < REFRESH_INTERVAL_MS) {
    return;
  }
  cachedStreamers = await getAllStreamers();
  nextStreamerIndex = 0;
  lastRefreshAt = now;
}
...
async function runSchedulerIteration() {
  ...
  try {
    ...
    const lockAcquired = await acquirePollLock(streamerId, schedulerId, LOCK_TTL_SECONDS);
    if (!lockAcquired) {
    ...
    try {
      ...
      await releasePollLock(streamerId, schedulerId);
    }
...
function startPolling() {
  ...
  refreshStreamersCache(true)
    .catch(error => {
      console.error('[DA Poll] Failed to refresh streamers cache on start:', error);
    })
    .finally(() => {
