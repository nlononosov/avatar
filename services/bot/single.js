...
let racePlanGameTickInterval = null; // Interval for race plan game tick
let racePlanObstacleInterval = null; // Interval for spawning obstacles in race plan
...
// Вспомогательно
function clampLane(l) { return Math.max(0, Math.min(4, l|0)); }
...
function startRacePlan(client, channel, settings = {}) {
  ...
  logLine(`[bot] Starting race plan in channel: ${channel} (normalized: ${normalizedChannel})`);
  logLine(`[bot] Settings:`, JSON.stringify(settings));
  logLine(`[bot] Client object: ${typeof client}, exists: ${client ? 'yes' : 'no'}`);
  logLine(`[bot] Channel: ${channel}, Normalized: ${normalizedChannel}`);
  ...
  if (!normalizedChannel) {
    logLine(`[bot] ERROR: No channel provided to startRacePlan!`);
    return;
  }
  
  // Reset previous game if one is active (allow restarting)
  if (racePlanState.isActive) {
    logLine(`[bot] Race plan is active (started: ${racePlanState.gameStarted}, finished: ${racePlanState.gameFinished}). Resetting...`);
    
    // Stop any ongoing game timers
    if (racePlanGameTickInterval) {
      clearInterval(racePlanGameTickInterval);
      racePlanGameTickInterval = null;
      logLine(`[bot] Cleared game tick interval`);
    }
    
    if (racePlanObstacleInterval) {
      clearInterval(racePlanObstacleInterval);
      racePlanObstacleInterval = null;
      logLine(`[bot] Cleared obstacle spawn interval`);
    }
    ...
    emitOverlay('racePlanEnd', { reset: true }, normalizedChannel);
    
    logLine(`[bot] Previous race plan state has been reset`);
  }
  ...
  // Announce game with settings
  logLine(`[bot] About to send announcement message to channel: ${normalizedChannel}`);
  if (!client || !client.say) {
    logLine(`[bot] ERROR: client or client.say is not available!`);
    return;
  }
  client.say(normalizedChannel, `✈️ Кто хочет участвовать в гонке на самолетах, отправьте + в чат! У вас есть ${registrationTime} секунд! (${minParticipants}-${maxParticipants} участников)`).catch(err => {
    logLine(`[bot] say error: ${err.message}`);
    logLine(`[bot] Full error: ${JSON.stringify(err)}`);
  });
  logLine(`[bot] Race plan announced in channel: ${normalizedChannel}`);
  
  // Start registration timer
  setTimeout(() => {
    if (racePlanState.participants.size < minParticipants) {
      client.say(normalizedChannel, `⏰ Время вышло! Недостаточно участников (${racePlanState.participants.size}/${minParticipants}). Гонка отменена.`).catch(err => {
        logLine(`[bot] say error: ${err.message}`);
        logLine(`[bot] Full error: ${JSON.stringify(err)}`);
      });
      racePlanState.isActive = false;
    ...
    if (racePlanState.participants.size > maxParticipants) {
      ...
      client.say(normalizedChannel, `🎯 Слишком много участников! Выбраны первые ${maxParticipants} участников.`).catch(err => {
        logLine(`[bot] say error: ${err.message}`);
        logLine(`[bot] Full error: ${JSON.stringify(err)}`);
      });
      touchOverlayState();
    ...
    logLine(`[bot] About to call startRacePlanCountdown with client: ${typeof client}, channel: ${normalizedChannel}`);
    startRacePlanCountdown(client, normalizedChannel);
    touchOverlayState();
  }, registrationTime * 1000);
}
...
function joinRacePlan(userId, displayName, client, channel) {
  ...
  racePlanState.levels.set(userId, 2); // Start at middle level (center of 5 lanes: 0-4)
  racePlanState.lives.set(userId, 3); // Start with 3 lives

  // Добавляем в Game состояние
  Game.players.set(userId, {
    lane: 2, // middle lane (center of 5 lanes: 0-4)
    lives: 3,
    out: false,
...
function startPlaneRaceMonitoring(client, channel) {
  ...
  racePlanObstacleInterval = setInterval(() => {
  ...
  racePlanGameTickInterval = setInterval(() => {
  ...
  logLine(`[bot] Game tick started, interval ID: ${racePlanGameTickInterval}`);
}
...
function finishRacePlan(winnerName, client, channel) {
  ...
  if (racePlanGameTickInterval) {
    clearInterval(racePlanGameTickInterval);
    racePlanGameTickInterval = null;
    logLine(`[bot] Cleared game tick interval on finish`);
  }
  
  if (racePlanObstacleInterval) {
    clearInterval(racePlanObstacleInterval);
    racePlanObstacleInterval = null;
    logLine(`[bot] Cleared obstacle spawn interval on finish`);
  }
