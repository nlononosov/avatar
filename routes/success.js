        ...
        <div class="card">
          ...
          <div style="margin-top: 15px; padding: 15px; background: rgba(245, 158, 11, 0.1); border: 1px solid rgba(245, 158, 11, 0.3); border-radius: 8px;">
            <p style="margin: 0; color: #f59e0b; font-size: 14px; font-weight: 600;">
              ⚠️ Для подключения DonationAlerts сначала авторизуйтесь через Twitch
            </p>
            <a href="/auth/twitch/init" class="btn btn-warning" style="margin-top: 10px; display: inline-flex;">
              <span>🔑</span>
    ...
    document.getElementById('connectDABtn').onclick = async () => {
      ...
      if (daStatus.connected) {
        // Disconnect
          ...
          const response = await fetch('/api/donationalerts/disconnect', { 
            method: 'POST',
            headers: {
              'X-CSRF-Token': csrfToken
            }
        ...
        if (!uid || uid === 'undefined') {
          showNotification('❌ Сначала авторизуйтесь через Twitch!', 'error');
          setTimeout(() => {
            window.location = '/auth/twitch/init';
          }, 2000);
          return;
        }
