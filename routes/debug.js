...
function registerDebugRoutes(app) {
    ...
    try {
      ...
      res.json({ 
        ...
        users: users.map(u => ({
          ...
          login: u.login,
          da_user_id: u.da_user_id,
          da_username: u.da_username
        }))
      });
    } catch (error) {
  ...
  app.post('/debug/create-avatar', (req, res) => {
    try {
      const { twitch_user_id } = req.body;
      
      if (!twitch_user_id) {
        return res.status(400).json({ error: 'Требуется twitch_user_id' });
      }
      ...
      saveOrUpdateAvatar(twitch_user_id, avatarData);
      
      res.json({ 
        success: true, 
        message: `Аватар создан для пользователя ${twitch_user_id}`,
        avatarData
      });
    } catch (error) {
      res.status(500).json({ error: error.message });
  ...
  // Прямая отправка события avatar:show
  app.post('/debug/send-avatar-show', (req, res) => {
    try {
      const { streamer_id, twitch_user_id, display_name } = req.body;
      
      if (!streamer_id || !twitch_user_id) {
        return res.status(400).json({ error: 'Требуется streamer_id и twitch_user_id' });
      }

      const { getAvatarByTwitchId } = require('../db');
      const avatarData = getAvatarByTwitchId(twitch_user_id);
      ...
      emitToStreamer(streamer_id, 'avatar:show', {
        ...
        displayName: display_name || 'Test User',
        color: null,
      ...
      res.json({ 
        success: true, 
        message: `Событие avatar:show отправлено для пользователя ${twitch_user_id}`,
        avatarData
      });
    } catch (error) {
      res.status(500).json({ error: error.message });
  ...
  app.get('/debug/da-status/:streamer_id', (req, res) => {
    try {
      const { streamer_id } = req.params;
      const { getStreamerDA } = require('../db');
      
      const daData = getStreamerDA(streamer_id);
      ...
      res.json({
        ...
        da_connected: !!daData,
        da_data: daData ? {
          da_user_id: daData.da_user_id,
          has_access_token: !!daData.da_access_token,
          has_refresh_token: !!daData.da_refresh_token,
          access_token_length: daData.da_access_token?.length || 0,
          refresh_token_length: daData.da_refresh_token?.length || 0,
          expires_at: daData.da_expires_at
        } : null
      });
    } catch (error) {
      res.status(500).json({ error: error.message });
  ...
  // Тестирование матчинга пользователя
  app.post('/debug/test-user-matching', (req, res) => {
    try {
      const { username, da_user_id } = req.body;
      const { findUserByDAUserId, findUserByNormalizedLogin } = require('../db');
      
      let user = null;
      let matchMethod = null;
      
      // Тестируем матчинг по da_user_id
      if (da_user_id) {
        user = findUserByDAUserId(String(da_user_id));
        if (user) {
          matchMethod = 'da_user_id';
        }
      }
      
      // Тестируем матчинг по username
      if (!user && username) {
        user = findUserByNormalizedLogin(username);
        if (user) {
          matchMethod = 'normalized_username';
        }
      }
      
      res.json({
        success: true,
        input: { username, da_user_id },
        matched: !!user,
