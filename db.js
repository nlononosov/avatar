...
const pool = new Pool({
  ...
  connectionTimeoutMillis: PGPOOL_CONNECTION_TIMEOUT ? Number(PGPOOL_CONNECTION_TIMEOUT) : 10000
});
...
const initPromise = initialize().catch(error => {
  ...
  console.error('[db] Please check if PostgreSQL is running and DATABASE_URL is correct');
  console.error('[db] Current DATABASE_URL:', DATABASE_URL);
...
async function initialize() {
  ...
  await withClient(async client => {
    ...
    try {
      ...
      console.log('[db] All tables created, committing transaction...');
      await client.query('COMMIT');
      console.log('[db] Transaction committed successfully');
    } catch (error) {
      console.error('[db] Error during initialization, rolling back:', error);
      await client.query('ROLLBACK');
      throw error;
    }
  });

  console.log('[db] Initializing gifts...');
  await initializeGifts();
  console.log('[db] Initializing locked skins...');
  await initializeLockedSkins();
  console.log('[db] Clearing old gifts...');
  await clearOldGifts();
...
async function withClient(fn) {
  ...
  try {
    ...
    console.error('[db] Error in withClient:', error.message);
    throw error;
  } finally {
    if (client) {
      client.release();
...
async function run(sql, params = []) {
  try {
    ...
    await initPromise;
    console.log('[db] Initialization complete, executing query...');
...
async function initializeGifts() {
  ...
  for (const gift of gifts) {
    // Используем pool напрямую, чтобы избежать deadlock с initPromise
    const existing = await pool.query('SELECT id FROM gifts WHERE id = $1', [gift.id]);
...
async function initializeLockedSkins() {
  try {
    // Используем pool напрямую, чтобы избежать deadlock с initPromise
    const existing = await pool.query('SELECT COUNT(*)::int AS count FROM locked_skins');
    if (existing.rows && existing.rows[0] && existing.rows[0].count === 0) {
      const lockedSkins = [
...
async function saveOrUpdateUser(user) {
  console.log(`[db] saveOrUpdateUser called for ${user.twitch_user_id}`);
  const now = Math.floor(Date.now() / 1000);
  
  console.log(`[db] Inserting/updating user ${user.twitch_user_id}...`);
  await run(
    `INSERT INTO users (
  ...
  console.log(`[db] User ${user.twitch_user_id} saved to users table`);

  try {
    console.log(`[db] Checking for existing avatar for ${user.twitch_user_id}...`);
    ...
    if (!existingAvatar) {
      console.log(`[db] Creating default avatar for ${user.twitch_user_id}...`);
      await run(
      ...
      console.log(`[db] Avatar already exists for ${user.twitch_user_id}`);
    }
  } catch (avatarError) {
    console.error(`[db] Error creating avatar for user ${user.twitch_user_id}:`, avatarError.message);
  }

  console.log(`[db] Updating user cache for ${user.twitch_user_id}...`);
  try {
    ...
    // ignore cache errors
    console.log(`[db] Cache update error (ignored): ${error.message}`);
  }
  
  console.log(`[db] saveOrUpdateUser completed successfully for ${user.twitch_user_id}`);
}
...
async function giveGiftToUser(twitchUserId, giftType, giftId, count = 1) {
  // Нормализуем тип подарка (убираем префикс gift_)
  const normalizedType = giftType.startsWith('gift_') ? giftType.replace('gift_', '') : giftType;
  
  // Нормализуем ID подарка (убираем префикс gift_TYPE_, оставляем только число)
  let normalizedId = giftId;
  if (giftId.startsWith('gift_')) {
    // Удаляем префикс вида gift_common_, gift_uncommon_, gift_rare_
    normalizedId = giftId.replace(/^gift_(common|uncommon|rare)_/, '');
  }
  
  console.log(`[db] giveGiftToUser: type=${giftType} -> ${normalizedType}, id=${giftId} -> ${normalizedId}`);
  await addGiftToUser(twitchUserId, normalizedType, normalizedId, count);
  return true;
}
